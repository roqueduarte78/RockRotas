// Web Audio API Sound Synthesizer for Driver Alerts
export function playDetourAlertSound() {
  try {
    ...
    // Friendly 2-tone notification chime (D5: 587.33Hz -> A5: 880Hz)
...
// Discreet proximity chime when driver is within 500m of the next stop
export function playProximityAlertSound() {
  try {
    ...
    // Subtle, elegant 2-note bell chime (F5: 698.46Hz -> C6: 1046.5Hz) with soft decay
    const osc1 = ctx.createOscillator();
    const gain1 = ctx.createGain();
    osc1.type = 'sine';
    ...
    console.warn('Proximity chime error:', e);
  }
}

// Success chime when stop is completed
